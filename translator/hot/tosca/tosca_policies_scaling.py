...
class ToscaAutoscaling(HotResource):
    ...
    def _handle_nested_template(self, resources):
        ...
        template, parameters, deps = self.create_substack_from_servers(servers_to_scale, resources)

        substack_props = {'type': template}
        if parameters:
            substack_props['properties'] = parameters
        ...
        asg_props['resource'] = substack_props
    ...
    def handle_properties(self, resources):
        if self.type == "OS::Heat::AutoScalingGroup" or self.type == 'OS::Senlin::Policy':
            return

        self.properties = {}
        self.properties["auto_scaling_group_id"] = {
            'get_resource': self.policy.name
        }
        ...
        for resource in resources:
            if resource.name in self.policy.targets and resource.type not in SCALING_RESOURCES:
